...
void entry(unsigned long magic, unsigned long addr) {
    ...
    if (CHECK_FLAG(mbi->flags, 3)) {
        ...
        while (mod_count < mbi->mods_count) {
            start_of_file = mod->mod_start; // this is where the file system address will start
    ...
    initialize_keyboard();
    init_filesys(start_of_file);
    terminal_init();
    initialize_rtc();
    initialize_pit();
    //init_filesys(start_of_file); // add initialiazation of the file system
    //terminal_init();
    clear(); 
    clearVidBuf(); 
    sti();
